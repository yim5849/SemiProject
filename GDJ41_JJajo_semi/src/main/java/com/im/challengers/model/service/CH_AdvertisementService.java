...
/* ================================광고 번호로 광고 찾기============================================ */		
	...
	public CH_Advertisement searchAdvertisement(int adNo) {
		
		Connection conn=getConnection();
		CH_Advertisement ad = dao.searchAdvertisement(conn, adNo);
		close(conn);
		return ad;
...
/* ====================================광고 수정================================================ */
	...
	public int updateAdvertisement(CH_Advertisement ad) {
		
		Connection conn=getConnection();
		int result = dao.updateAdvertisement(conn, ad);
		if(result>0) commit(conn);
		else rollback(conn);
		close(conn);
		return result;
