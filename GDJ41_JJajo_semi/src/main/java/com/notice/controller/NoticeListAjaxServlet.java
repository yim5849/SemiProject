...
int curPage;
		...
		if(pageStart==1) {
			...
			pageBar="<a href='javascript:fn_noticepage("+(pageStart-1)+","+numPerPage+");'>[이전]</a>";
		}
		...
		while(!(pageStart>pageEnd||pageStart>totalPage)) {
			if(curPage==pageStart) {
				...
				pageBar+="<a href='javascript:fn_noticepage("+pageStart+","+numPerPage+");'>"+pageStart+"</a>";
			}
			pageStart++;	
		}
		...
		if(pageStart>totalPage) {
			...
			pageBar+="<a href='javascript:fn_noticepage("+pageStart+","+numPerPage+")'>[다음]</a>";
		}
