...
CH_Challengers ch = (CH_Challengers)request.getAttribute("challengers");
...
<section>
    ...
    <form action='<%=request.getContextPath()%>/challengers/ch_update_end.do'
      ...
      <div class="container">
        <div class="row row-cols-2">
          ...
          <div class="col-4">
            <div class="mb-3">
              <input type="text" class="form-control" id="ch_title_"  name="ch_title" 
              placeholder="챌린져스 제목을 입력하세요(10글자 미만 입력(공백포함))" value="<%=ch.getTitle()%>">
            </div>
          </div>
          <div class="col" style="text-align: center; font-size: 20px; color: lightseagreen;">챌린져스 이미지 등록</div>
          <div class="col-4">
            <div class="mb-3">
              <input class="form-control"  name="ch_upfile" type="file" id="formFile"><span><%=ch.getFilepath() %></span>
              <input type="hidden" name="ch_orifile" value="<%=ch.getFilepath()%>">
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="col-4">
            <div class="mb-3">
              <textarea class="form-control" id="ch_content_"  name="ch_content" rows="5" style="resize:none;" 
              	placeholder="** 해당 챌린지를 소개하는 글을 작성해주세요! (82글자 미만 입력(공백포함))**" ><%=ch.getContent() %></textarea>
            </div>
          </div>
          <div class="col"></div>
          <div class="col"></div>
	...
	<input type="hidden" name="ch_challengersNo" value="<%=ch.getChallengersNo()%>">
      <br>
      <br>
